...
export function EmptyState({ icon: Icon, title, description, action, variant = 'default' }: EmptyStateProps) {
  return (
    <div className={cn(
      ...
      variant === 'error' ? "bg-destructive/5 border-destructive/20" : "bg-slate-50/50"
    )}>
      <div className={cn(
        "flex h-12 w-12 items-center justify-center rounded-full mb-4",
        variant === 'error' ? "bg-destructive/10 text-destructive" : "bg-slate-100 text-slate-500"
      )}>
        <Icon className="h-6 w-6" />
      </div>
      <h3 className="mb-1 text-lg font-semibold tracking-tight text-slate-900">{title}</h3>
      {description && (
